...
bool DepthShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"./shader/depth.vs", L"./shader/depth.ps");
	if(!result)
	{
		return false;
	}
...
bool DepthShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "DepthPixelShader", "ps_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, 
