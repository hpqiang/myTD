...
//	m_CubeModel = new ModelClass;
	...
	result = ((CubeModelClass *)m_CubeModel)->Initialize(m_D3D->GetDevice()); // , m_D3D->GetDeviceContext() ); // , "./data/cube.txt", L"./data/wall01.dds");
//	result = m_CubeModel->Initialize(m_D3D->GetDevice(), L"./data/wall01.dds");
	if(!result)
	{
	...
	result = m_SphereModel->Initialize(m_D3D->GetDevice(), "./data/sphere.txt", L"./data/ice.dds");
	if(!result)
	{
	...
	result = m_GroundModel->Initialize(m_D3D->GetDevice(), "./data/plane01.txt", L"./data/metal001.dds");
	if(!result)
	{
	...
	m_GroundModel->SetPosition(0.0f, 1.0f, -1.0f);
