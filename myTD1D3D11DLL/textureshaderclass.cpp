...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, /*WCHAR**/LPCWSTR vsFilename, /*WCHAR**/LPCWSTR psFilename)
{
	...
	result = D3DX11CompileFromFileW(vsFilename, NULL, NULL, "TextureVertexShader", "vs_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, 
								   &vertexShaderBuffer, &errorMessage, NULL);
	if(FAILED(result))
	{
	...
	result = D3DX11CompileFromFileW(psFilename, NULL, NULL, "TexturePixelShader", "ps_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, 
								   &pixelShaderBuffer, &errorMessage, NULL);
	if(FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
	...
	//LPVOID bp = vertexShaderBuffer->GetBufferPointer();
	//size_t size = vertexShaderBuffer->GetBufferSize();
	result = device->CreateVertexShader(vertexShaderBuffer->GetBufferPointer(), vertexShaderBuffer->GetBufferSize(), NULL, &m_vertexShader);
	//result = device->CreateVertexShader(bp, size , NULL, &m_vertexShader);
	if(FAILED(result))
	{
		return false;
	}
...
void TextureShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, /*WCHAR**/LPCWSTR shaderFilename)
{
	char* compileErrors;
